...
async function abrirParamItem(key, id) {
  ...
  if (key === 'areas') {
    ...
    if (def.tieneCuentaContable) {
      var opcCuentas = [];
      try {
        const ctas = await api('cont_cuentas','GET',null,'?codigo=like.1.1.01*&estado=eq.ACTIVA&permite_movimiento=eq.true&order=codigo.asc&select=id_cuenta,codigo,nombre' + (_empresaActiva ? '&id_empresa=eq.'+_empresaActiva.id_empresa : ''));
