...
let _contMoneda = null; // se inicializa con moneda_principal de la empresa
...
async function contAbrirAsiento(id) {
  ...
  if (id) {
    ...
    contLineasAsiento = lineas.map(function(l){ return { id_cuenta: l.id_cuenta, descripcion: l.descripcion||'', debe_usd: l.debe_usd, haber_usd: l.haber_usd }; });
  } else {
    ...
    + contLineasAsiento.map(function(l, i) {
        return '<tr>'
          ...
          + (function(){
              ...
              const esDeudora = !cInfo || cInfo.naturaleza === 'DEUDORA';
              if (!esDeudora) return '<td style="padding:4px;text-align:center;color:var(--suave);font-size:11px">—</td>';
              return '<td style="padding:4px"><input type="number" value="' + (parseFloat(l.debe_usd||0).toFixed(2)) + '" min="0" step="0.01" onchange="contLineasAsiento[' + i + '].debe_usd=parseFloat(this.value)||0;contLineasAsiento[' + i + '].haber_usd=0;contRenderLineasForm()" style="width:100%;background:var(--gris2);border:1px solid var(--borde);color:#22c55e;font-family:var(--font-mono);font-size:12px;padding:6px 8px;border-radius:4px;outline:none;text-align:right"></td>';
            })()
          + (function(){
              const cInfo2 = contCuentasCache.find(function(x){ return x.id_cuenta === l.id_cuenta; });
              const esAcreedora = cInfo2 && cInfo2.naturaleza === 'ACREEDORA';
              if (!esAcreedora) return '<td style="padding:4px;text-align:center;color:var(--suave);font-size:11px">—</td>';
              return '<td style="padding:4px"><input type="number" value="' + (parseFloat(l.haber_usd||0).toFixed(2)) + '" min="0" step="0.01" onchange="contLineasAsiento[' + i + '].haber_usd=parseFloat(this.value)||0;contLineasAsiento[' + i + '].debe_usd=0;contRenderLineasForm()" style="width:100%;background:var(--gris2);border:1px solid var(--borde);color:#f87171;font-family:var(--font-mono);font-size:12px;padding:6px 8px;border-radius:4px;outline:none;text-align:right"></td>';
            })()
...
async function contGuardarAsiento() {
  ...
  if (Math.abs(totalDebe - totalHaber) > 0.01) {
    errEl.textContent = 'El asiento no cuadra. Debe = $ ' + fmtUSD(totalDebe) + ' | Haber = $ ' + fmtUSD(totalHaber);
  ...
  try {
    ...
    for (let i = 0; i < contLineasAsiento.length; i++) {
      ...
      await api('cont_asiento_lineas','POST',{
        id_asiento: parseInt(asientoId), id_cuenta: l.id_cuenta,
        descripcion: l.descripcion||null, orden: i+1,
        debe_usd:  parseFloat(l.debe_usd||0),  haber_usd: parseFloat(l.haber_usd||0),
        debe_ves:  parseFloat(l.debe_usd||0)*tasa, haber_ves: parseFloat(l.haber_usd||0)*tasa,
      });
    }
