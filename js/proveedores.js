...
async function verFichaProveedor(id) {
  ...
  document.getElementById('ficha-prov-contenido').innerHTML =
    ...
    + (aceptaTransferenciaActual && (p.pm_id_banco || p.pm_celular) ? '<div style="grid-column:1/-1;margin-top:12px;padding-top:12px;border-top:1px solid var(--borde)"><div style="font-size:10px;color:var(--naranja);letter-spacing:1px;text-transform:uppercase;margin-bottom:10px;font-weight:600">📱 Pago Móvil</div>'
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px">'
      + '<div><div style="font-size:13px;font-weight:700;color:var(--suave);letter-spacing:0.5px;text-transform:uppercase;margin-bottom:3px">Banco</div><div style="font-size:14px">' + ((_provBancosCache||[]).find(function(b){return b.id===p.pm_id_banco;})?.nombre || '—') + '</div></div>'
      + '<div><div style="font-size:13px;font-weight:700;color:var(--suave);letter-spacing:0.5px;text-transform:uppercase;margin-bottom:3px">C.I. / R.I.F</div><div style="font-size:14px;font-family:var(--font-mono)">' + (p.pm_ci||'—') + '</div></div>'
      + '<div><div style="font-size:13px;font-weight:700;color:var(--suave);letter-spacing:0.5px;text-transform:uppercase;margin-bottom:3px">N° Celular</div><div style="font-size:14px;font-family:var(--font-mono)">' + (p.pm_celular||'—') + '</div></div>'
      + '</div></div>' : '')
    + '</div>';
